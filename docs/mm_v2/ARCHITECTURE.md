...
2026-09-13 場次193558後：ReadStream唯讀first_stream_failure只記固定分類，正常close／取消不算故障、後續錯誤不蓋初因。MM記錄當次book等待／watermark／transport及source診斷，取消或逾時清除可用handoff但保留數值到記錄完成，下一cycle重設。正常授權新增require_aligned_book；只有完整帳戶證明已通過、generation未變、stream與最新book仍strict有效，且缺book因等待逾時／超watermark時，才共用原account audit的10s／一次retry重新讀cash、orders與trades，逐項admission並維持fill／funding去重。不可重試invalid／斷線或超額，不延長期限、不放寬book界線、不重送交易；退出帳戶查核維持可無book，無可信IOC行情時如實殘量失敗。Raw完整流程含持續等待、nonce斷鏈、EOF、同步成交及暫時對齊恢復，還原舊policy必被同一驗收抓出；最終驗證見[EXPERIMENT_LOG](EXPERIMENT_LOG.md)。

2026-09-13 測試契約增補：完整鏈以原CLI／session／manager／port／account／ledger／SDK nonce與sign/send／HTTP反序列化／ReadStream組合，僅native及外部傳輸使用合成fixture。Windows入口保留真60s quarantine／Proactor／CTRL_C_EVENT，成功必須在關閉test job前證明整樹自然退出。PS1在finally取得並傳回Python真實exit code，防止Ctrl+C造成window預設1、外層0的誤報。独立venue、原code object、前後source/config指紋與歷史缺陷反例共同驗收；冷匯入後也須恢復原連線default。最終646項V2全PASS，詳見[EXPERIMENT_LOG](EXPERIMENT_LOG.md)及[主計畫§19.9](../CODEX_MM_VOLUME_FIRST_V2_REBUILD_PLAN.md)。

2026-09-13 場次000352後目前狀態：原場取消缺證後final authenticated flat／2單，formal economics unavailable；使用者其後親自確認0/0，本輪未執行私人API查核。新generic signer/provider診斷、4次history無exact terminal及2次recovery仍pending，尚不能區分最初是送出前或送出後失敗；真SDK本地隔離重現確認這些階段可能得到相同generic結果，整場sendTx計數亦不能替代逐attempt證據。本地已補真SDK sign／send階段、精確no-send／nonce契約及僅供cleanup的持續鎖定。清理前額外5000REST／7WS／0TX準入涵蓋串流與REST帳戶查核，保留原退出預留與10s／30s期限；fresh account可支持撤除known orders而不另要求行情，IOC仍走原fresh market檢查。MM create也移除DNS及文字429錯誤恢復，default/Grid行為保留。獨立審查、全專案回歸及修正契約補測完成，632項V2均有通過證據；其餘12項維持既有基線。只有確定未送出的證據能支持相應恢復；可能已送出者仍須原exact terminal／ownership／deadline及API gates，不以generic err tuple消除不確定性或重送。具體run與驗證統一見[EXPERIMENT_LOG](EXPERIMENT_LOG.md)，下列前批完成內容屬歷史契約與證據。

2026-09-12 場次224839後：known-cancel recovery仍只授權同一次exit-only操作，最初即消耗marker；若第一讀缺pending terminal且其餘fault／mutation／known-ID／registry邊界不變，在原10s內等待0.5s後最多再讀一次，每讀均經原API admission。首讀額度拒絕仍傳遞；額外可選讀拒絕記固定secondary原因並返回未恢復，不改原cancel原因。仍active／absence不授權重送，只有exact terminal、registry清空與健康known state才恢復原有界收尾；正常報價保持失敗。Manager與診斷共用strict receipt拒絕判定，ReconcileAction附allowlist數值；REST MM-only最多保留兩個取消ID的submission類別及history計數，確認後清除。Telemetry只輸出目前pending相符action及固定recovery計数，無raw payload；同步診斷故障不能中斷已成功的交易處理。Analyzer分列historical typed final report／snapshot事實，incomplete replay不能以預設false/null覆蓋，也不能升級economics。證據及回歸統一見[EXPERIMENT_LOG](EXPERIMENT_LOG.md)。

2026-09-12 場次215907後：MM opt-in的cancel handoff保留REST取得的完整CANCELED／partial CANCELED／FILLED／EXPIRED／REJECTED DTO，Adapter直接返回，禁止先清registry再用amount0／price=None占位回覆代替。Pending marker與terminal cache一直保留至manager以原ID／side／size／price等契約驗證並確認；缺終態不重送signer，已cached的重讀不新增請求。MM confirm只消費exact exchange ID，禁止client ID別名清掉另一pending；Grid/default原bool／placeholder及alias行為不變。真Adapter→REST→manager與session的雙向反覆60s到期撤換均有離線回歸；詳細實盤失敗及驗證見[EXPERIMENT_LOG](EXPERIMENT_LOG.md)。

2026-09-12 場次205914後：執行層區分可補查的known cancellation故障與通用HALTED。`can_reconcile_known_cancellations`只授予exit-only只讀資格，拒絕submission ambiguity、unowned／unknown及外來取消registry，原normal permission不變。取消專屬ID集合在一次sync前消耗；原deadline／單次10s內必須證明所有該ID terminal、registry空、mutation generation及known-ID集合不變，剩餘slots健康，才能解除這一次取消鎖停。仍active或無terminal不構成重送權限。正常loop故障後只收尾，bounded cancellation自身延遲terminal亦最多補查一次；所有account證據、original3IOC／固定價格／30s及API gates保留。FailureDiagnostic新增目前相符cancel batch的固定碼，無raw provider payload；analyzer provenance採runner實際 `.jsonl.budget.json` 路徑。

取消／補查跨await另保留fault generation；新generic halt不得改標為取消可恢復，也不得被舊sync結果清除。取消終態匹配須exact exchange ID、相容client ID、相同原量與價格及不增加的remaining，不能由另一個已known ID的相同client替代；direct／cache／history／取消中更新沿用同一驗證。其他submission resolution不因這次取消修正變更匹配規則。

2026-09-12 fee-cover分析後更新：InventoryGovernor只在原joint capacity兩側均低於minimum、且非flat時，額外搜尋 `min(abs(position), order_size)` 內的正常減倉容量。沿用同一allowed公式，未撤舊單、current drawdown／gap及全部exit／maker reserve仍計入；嚴格風險不等式與hard／hold／loss／deadline／stop路徑不變。通過時保留正常quote policy，不把它改為入場成本鎖利或breakeven exit。`GovernorDiagnostic`從當次真正風險計算產生原因／容量／reserve，未計算欄位null，remaining headroom尚未扣reserve。

`OrderEvidence`記錄已確認owned ID與實際提交意圖、本地提交／確認區間及pre-submit external market；在FillAccounting之前emit，IOC另須exact terminal。缺失或無法建立的證據不影響清理，也不回填ledger reference。市場snapshot另攜帶原stream source timestamp；REST沒有該source證明時為null。Shared Lighter read stream提供預設關閉的public BBO observer，V2於初始account記帳後開啟、final report前關閉，以最多4Hz記錄原source／receipt／nonce及BBO；無額外請求、不改Grid預設或source有效性。這些公共樣本可能含own orders，不取代external fill reference。

既有session analyzer用實際0→0持倉邊界分組，不拆翻倉fill；normal／passive-reducing maker分類需要此前相容order證據，缺失保持unclassified，cost按平倉fill認列而非獨立策略PnL。公開markout以fill source clock對齊1s／5s，列明0.25s樣本容差與coverage；本地submit→receipt不是exchange working age。Formal economics仍依原完整final accounting，交易診斷不使失敗session升級通過。Runner啟動前在原budget sidecar保存commit／dirty與allowlisted有效設定，無Git或歷史缺檔明示不可用，不以現版本回填舊場。
...
Quote revision按side判定：只撤過期／需改價／容量或reduce-only權限改變的owned側，保留仍被fresh風險授權的對側及原排隊時間。每次撤單均需該側exact terminal，再刷新帳戶與風險；保留側若因cancel-fill race變得不合法，必須先撤後新增。最多處理原兩側，不重置10s deadline；fee變更或保留價格與新單cross仍可能全撤。`cancel_managed_orders`省略sides仍是全撤，cleanup不受此優化削減。

退出時BBO越過固定IOC限價不代表不確定execution：保留原限價提交reduce-only IOC，已證明的零成交terminal仍回傳fresh residual供原三次／期限內處理。Stale／untrusted book、unknown order或不確定mutation仍阻擋新增委託；市場持續界外則明示attempts exhausted與殘倉，不逐次放寬價格。
...
成本分母不含 taker turnover；funding 不美化 fee cover，入出金不算獲利。零分母回 `None`。Live fill必須帶交易所逐筆`realized_pnl`（有限Decimal、納入同ID不可變證據及JSONL），以此累計gross並分配partial close的剩餘cost basis；完整flat也不能用模型成交現金流覆蓋交易所結算。不把對帳差額補成funding、不猜量化位數；結算rounding若不能精確拆成spread／drift，decomposition標incomplete。舊JSONL全程缺該欄位可維持平均成本模型重播，同session不混用模型與交易所來源；live缺欄拒絕。Drawdown 為觀測到的 transfer-neutral marked equity peak-to-trough；inventory average/P95 依整場時間加權、包含 cooldown/flatten。Quote uptime 是至少一側 maker order **實際 working** 的觀測時間聯集，不能使用 proposed quote 或雙邊重複計時。MarkEvent另保存已證明的quote_sides，ledger報buy／sell／two-sided seconds，滿足union=buy+sell−two-sided；舊事件只知quoting而無side時，側別統計為None，不從quote plan補造。這些時間以觀測點分段，未宣稱具有exchange事件時間精度；網路dry的真實working時間仍為0。
...
一條 exclusive JSONL 保存 typed events；SDK logging 在 CLI run 範圍停用並恢復，避免 raw responses／credentials 進入輸出。 `--progress`只在狀態改變、fill、退出／exit ID、錯誤及每60s heartbeat輸出；合併quote內部暫態、重用session現有account快照，保留觀測年齡，無额外API。Console不是財務journal，開關不改JSONL bytes，關閉console不阻斷cleanup。Phase 7 前提：確認獨占帳戶設定後才做唯讀 dry smoke → dry T3。REST consistency read 可能在正常 partial-fill race 時拒絕並觸發安全收尾，呼叫頻率／延遲／這類 false No-Go 的實際 liveness 尚未驗證；不能把離線 runner 通過解讀成連續實盤已穩定。
...
`ApiBudget`只計owned Python REST transports與owned WebSocket所有frames（含control／ping／pong／close），不記URL query、headers或body；未知endpoint拒絕。Live在connect/authenticate後、開stream前等待60s，使無法截取的native signer startup請求退出rolling window；stop會中斷等待。其他IP／L1 consumers不在本meter內，操作者須維持獨占。Runner另存exclusive `.jsonl.budget.json`，檔案已存在即拒絕，不覆寫證據。 同一sidecar記錄本地limits、起始tier／實際費率、startup／normal／exit REST weight，以及最多64筆API背壓退出的phase／exit ID／時間／成本。第3次本地背壓出現在同600s窗口內時，先完成既有清倉，之後停止重入並回報`api_backpressure_repeated`；finally仍做新0/0及帳務確認，失败不得偽裝成功。AccountReadRace另計；早停不能算完整經濟窗口。

Normal account audit、額外order read、OM sync及mutation前皆做admission；post-send terminal confirmation不被normal guard截斷。Stream audit起始檢查REST300／WS5，覆蓋下一REST gate之前的cash300；不將後面另有gate的trades600與terminal history100提前綁入；條件性REST在**實際查詢之前**再分步檢查：fees900（兩個authenticated endpoint600＋最多一個public funding round300）、settlement300、trades600、terminal history100。快取仍有效或沒有新交易／終態時，對應query與分步gate均省略；正常terms採上述28s刷新／30s上限，exit／final仍8s；原始request-start及exact帳戶證據保留。這些gate不是已扣款／持有的reservation，故metadata之後的trades/history不可省略自己的檢查，也不能將gate數字相加當作實際wire用量。任一步拒絕都不得發送該query；settlement只在成功後原子更新值和request-start，避免舊值被標成fresh。無stream的forensic雙讀不屬於300 audit上界，各實際fees/settlement/trades仍經既有callback。

Normal撤單按實際selected側數檢查REST412／WS0／TX1每側；412覆蓋MM terminal-only四次history400與SDK nonce初始／invalid-nonce刷新12。建單仍1400／8／2，不以取消的窄路徑套到建立。零managed空plan不預檢mutation；exit不走normal admission。
