...
下文描述設計契約及既有接線，不能視為全部已驗證。`260be69` 的離線review確認 cleanup、passive exit、POST_ONLY recovery、持續雙邊報價、own/book對齊、current drawdown reserve與final inputs freshness共七項缺口。後續授權修復已在worktree修正R1／R2六項，F5 own/book對齊及R3整體資料／預算仍未完成。具體重現、驗收與進度見 [rebuild plan §19](../CODEX_MM_VOLUME_FIRST_V2_REBUILD_PLAN.md#19-2026-09-05-review從目前-phase-7-接續)。
...
既有source-clock、完整live REST／WS budget、正常fill arrival race與Unified nonflat帳务的未驗證狀態仍保留。後续先修退出與quote lifecycle，再整合資料／預算，不能用flat dry替代live execution proof。經濟主指標已改固定完整時間的maker turnover；wall-hour、雙邊uptime、capital turnover及candidate聚合屬待實作報表，现有ledger欄位保留原含義。`scripts/analyze_mm_v2_session.py`尚未實作。
...
Book 使用 RH 官方 nonce 契約：initial full snapshot，後續 begin_nonce 必須等於前次 nonce，offset 單調增加；source timestamp 不得未來／倒退／超過 3s，receipt 亦不能超過 3s。主機時間不合即拒絕，不自動推算偏移。診斷記錄source age及wall/monotonic elapsed差異；source age仍在界線內但相鄰elapsed差額超過50ms也視為clock jump，停用行情並保留account cleanup transport。50ms是主機連續性界線，不是UTC誤差容忍；Windows／Linux共用此條件。Windows使用同域perf_counter/QPC作session、account、market及OM monotonic；source epoch時間仍獨立檢查。僅一個host wall-clock quantum內的future packet可在receiver等待一次（cap20ms），再執行原strict0..3000ms；等待前receipt保留，不接受仍future／已stale資料。完全相同且仍fresh的book/own snapshot可重讀，不重寫時間；不同內容仍须新嚴格遞增receipt。Book data failure 永久封鎖行情但保留正常 account transport；真正 transport／identity／protocol failure 全部失效。失敗收尾才明確關閉 read stream 並使用有界 REST 查證，market 仍不可用，不恢復報價或無價 IOC。Quote book必須在opening order request之後實際收到，且opening nonce ≤ book nonce ≤ closing nonce、兩端完整own orders相同；最多等待3s，不補時間戳、不盲扣舊book的own size。超時或超出兩端的book不能報價，但已成立的authenticated flat證明仍供cleanup使用。Aligned packet每次重用仍透過stream檢查其原始source timestamp和目前行情健康；較新的book fresh不會讓舊packet延壽，book fault也不能被healthy account transport掩蓋。Normal wrapper每次重新evaluate，僅同cycle／相同ownership-exposure key可共用fresh account；post-cancel必須新現金查證。3s cycle 不新增 YAML knob；stop 獨立記時，慢讀取不延後 30s 退出界線。最後仍需 bounded cleanup、獨立 authenticated postflight、ledger finalize 與 disconnect。
...
Live read-budget 仍為 No-Go：合法高改價離線 fixture 在加入 unsubscribe 前就曾達 rolling60 REST 26,700、WS 192（未含 ping/pong）；同一 fixture 換算現版 unsubscribe 握手約 308 WS/min，明確超出 200；不能把 flat dry 成功當成改價／terminal-history／最多三次 IOC 共存的 reserve 證明。進 Phase 8 前須以完整 mutation/reconciliation/exit 讀取路徑修正並驗證 reserve，且仍需逐場新 live＋bounded-flatten 授權。Inventory tuning 依計畫Phase9及§19 R4的可行／接近可行或最小可執行性修正條件進行；2h/4h/24h 依計畫順序，不沿用 V1 Gate A/B/C。V1 移除不改變這些 gate；shared Lighter 改動必須執行 Grid 回歸。

2026-09-05 R3 的去重只降低成本，沒有完成 API reserve：同一新高改價 fixture 的 WS 下限330→195；把 shared create lookup／cancel terminal／get_order 最低查詢補入後，REST下限35,600，terminal history合批後30,000。這些仍是來源碼成本下限，不是實際wire峰值；source/API同期到達、重試、三次IOC與final proof仍須納入admission。10min flat dry只驗證讀取路徑；詳細證據見EXPERIMENT_LOG及計畫§19.4。
