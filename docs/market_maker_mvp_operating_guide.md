...
## Gate C shadow live 非平倉收尾（2026-09-03）

本節取代先前「可繼續收集shadow calibration」的當前決策，但不改寫歷史Gate A／B dry證據。候選使用branch `feat/lighter-market-maker-mvp`、HEAD `0a377987c74148534003b021da93de649f28720e`、shadow live semantic config SHA-256 `AF2907FD28C9C6D4C6F3EB5EF7260467143E29D7DD88BBDB80C75DCDE4FC0BB7`；操作者提供campaign flat-loss budget `1 USDG`。本輪授權只涵蓋最長30分鐘maker-only shadow live，不包含active IOC、自動flatten、margin／leverage變更或第二輪live。

22:13:54–22:43:57執行30分鐘live，terminal uptime `1803.062s`、`344/344` cycles。唯一成交為authenticated maker SELL entry `0.00020 BTC @ 78925.0`；taker／active unwind皆`0`，completed fills／round trips皆`0/0`。行情在開空後明顯上漲；`active_unwind_enabled:false`且`max_session_loss_for_maker_exit:0`使executor只能把`POST_ONLY + reduce_only` BUY維持在不虧損的authenticated economic boundary `78905.7`，最終為`INVENTORY_HOLD`且無法自然成交。Terminal signed／audited／ledger position一致為short `0.00020`，audited open orders為`0`，unrealized PnL `-0.161420 USDG`，current／max drawdown為`0.163314 / 0.180454 USDG`。全部failed／consecutive、ambiguity／unresolved、reconciliation failure、unknown、mutation limiter、429、active、account-read、WS及controller hard counters均為`0`；所以這不是order lifecycle或runtime事故，而是maker-only退出政策在單邊行情下不收斂的**風險政策No-Go**。

Timed shutdown依規則取消退出單、不自動平倉；runner因authenticated boundary非flat而exit `1`。Immutable artifact `logs/market_maker_evidence/shadow_live_0a37798_20260903-2212.json`共181 records、2,645,569 bytes，SHA-256 `A0E6B51E7E96E6F2E422FCCEAD0F74E50A01AC9D6C490CD6DB6D74312A594503`；physical-last亦為max-ranked terminal。Integrity errors為`runner_failed / incomplete_authenticated_postflight / missing_final_authenticated_open_orders / missing_or_nonflat_final_signed_position`，writer依fail-closed契約刻意把`commit_sha`設為null。Campaign analyzer因此只列diagnostic、`included=0`、`recommendation_allowed:false`；不得以後續人工flat回填或改寫原artifact。

22:44兩次authenticated read-only postflight均確認runtime已消失、BTC short `0.00020 @ 78925.0`、open orders `0`。操作者其後自行平掉殘倉；Codex未送出cancel、recovery或flatten mutation。23:47:21及23:47:37兩次authenticated read-only檢查一致為position／orders／used collateral=`0/0/0`、flat equity `298.814557 USDG`。相對本輪baseline equity `299.079866`，保守flat-equity change為`-0.265309 USDG`；此損失計入campaign budget後剩餘`0.734691 USDG`。外部postflight與人工flat風險帳另存ignored record `logs/market_maker_evidence/shadow_live_0a37798_20260903-2212_postflight.json`，不冒充原run的authenticated terminal evidence。

當前結論固定為 **Gate C shadow calibration candidate_no_retry_without_change / final-flat已由操作者恢復 / 4h禁止啟動**。同candidate不得重跑，也不得以剩餘campaign budget視為安全豁免。若日後恢復作業，必須先提出能處理time-bounded non-flat的明確退出政策與授權邊界，完成offline／MM／必要full regression及fresh T3，再取得新的逐場live授權；在此之前不執行其他live。
