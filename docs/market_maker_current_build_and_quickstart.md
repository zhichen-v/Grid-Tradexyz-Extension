...
> 更新：2026-08-30 22:53 Asia/Taipei。這是「現在可用什麼、參數是什麼、如何安全導入」的入口；不可取代 [操作指南](market_maker_mvp_operating_guide.md) 的安全規則。逐輪證據見 [驗證歷史](market_maker_mvp_validation_history.md)，架構細節見 [MVP Pipeline](CODEX_MARKET_MAKER_MVP_PIPELINE.md)。
...
| Active inventory unwind | 預設關閉；IOC expiry mapping已修復。Live exact-terminal path已證明：active attempts／success=`3/3`、2 taker fills、1 partial、ambiguity`0`。但session loss cap可能在下一筆active mutation前安全停止並留下inventory，仍非production-ready |
| Final 4h validation | **Session-cap safe stop／4h未完成**：21:48:54開始，22:49:54第10輪因下一次active unwind預估會越過`0.10 USDG` session cap而在submission前停止；runtime0、open orders0，22:53證據為BTC long `0.00020` |

啟動前兩次authenticated preflight皆為BTC position／orders=`0/0`、used collateral=`0`、equity=`299.146456`。停止後22:53雙postflight皆reads PASS、orders`0`，但BTC long`0.00020`、used collateral`15.741580`、equity`299.047812`。本輪只有在`>=30` completed maker fills且authenticated natural-flat時才形成完整economic判定；本次17 completed且nonflat，只能記`incomplete_nonflat`。

## 2. 搭建摘要

資料與控制流：

`trusted external BBO / position` → `strategy quote` → `risk + inventory episode` → `single coordinator` → `order manager` → `Lighter adapter` → `authenticated audit / metrics`
...
| Active驗證overlay | 只供明確授權的獨立validation | `active_unwind_enabled: true`、maker-exit budget `0.05`、active after `300s`、有界IOC及episode/session loss cap |
...
| Episode／session unwind cap | n/a | `0.10 / 0.10 USDG` |
...
固定驗證overlay source SHA-256：`73F53128787FA6B84ECCA7C147E894E81FB790F97A09AAE5EE50825809D6267E`。Live期間copy SHA-256：`508BB0854B151D8C16BA34E39FA7C11387242E6B89C63AE183B405F12BB959E3`；停機後兩份皆恢復`dry_run:true`且同source SHA。Active base commit：`bc9e8a3e9ca8cfbd4ce215430627c4e76273c4ea`；IOC expiry修復後`lighter_rest.py` SHA-256：`419FE63D52DCD3B73EFA25AA51172DA685535B4E720AA71C670BF50AD6FEC7F3`。
...
停止後runtime0，live copy已恢復`dry_run:true`且與source同SHA。22:53兩次authenticated postflight皆identity／wallet／reads PASS、open orders`0`，並一致顯示BTC long`0.00020 @ 78707.9`、used collateral`15.741580`、equity`299.047812`。沒有新的flatten授權，因此未做更多mutation。只有17 completed maker fills且沒有final natural-flat checkpoint，正式economic state為`incomplete_nonflat`；負bps／fee cover是不利訊號，但不是達30-fill門檻後的正式GO／NO-GO。

## 8. 已知限制

- Active IOC exact-terminal path已證明，但session unwind cap可在新mutation前安全停止並留下inventory；若需求是「任何單邊行情都自動平掉」，現況尚未達成。不得為完成run而放寬cap。
- 本次只運行61分鐘、17 completed fills且final nonflat，沒有production promotion、完整4h或正式economic GO／NO-GO證據。
- 停機後仍有long`0.00020`；系統不會在未授權時自動flatten，新的current truth需fresh authenticated reads。
