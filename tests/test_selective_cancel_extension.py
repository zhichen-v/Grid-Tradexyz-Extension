...
class Rest:
    ...
    def __init__(self, ids, error=None, active=False):
        self.config = {"cancel_batch_size": 50}
        self.api_key_index = 2
        self.signer_client = Signer(error)
...
class LighterBatchTests(unittest.IsolatedAsyncioTestCase):
    async def test_51_ids_use_two_mutations_and_two_bulk_snapshots(self):
        ids = list(range(1, 52))
        rest = Rest(ids)
        with patch.object(lighter.asyncio, "sleep", new=AsyncMock()):
            report = await lighter.cancel_orders_batch(rest, "BTC", ids)
        self.assertEqual([len(x["tx_types"]) for x in rest.signer_client.sent], [50, 1])
        self.assertEqual(rest.open_reads, 2)
        self.assertEqual(rest.history_reads, 2)
        self.assertEqual(report.cancelled, {str(i) for i in ids})
        self.assertEqual([x["nonce"] for x in rest.signer_client.signed], list(range(10, 61)))

    async def test_transport_loss_is_uncertain_and_never_resubmitted(self):
        rest = Rest([101], ConnectionError("lost"))
    ...
    async def test_429_rolls_back_nonces_and_is_retryable(self):
        ...
        self.assertEqual(report.rejected, {"201": "HTTP 429", "202": "HTTP 429"})
...
class Engine:
    def __init__(self, ids):
        ...
        self.orders = {str(i): Order(i) for i in ids}
        ...
        self.coordinator = SimpleNamespace(state=SimpleNamespace(active_orders=dict(self.orders)))
    ...
    def _pending_keys_for_order(self, order):
        return [key for key, value in self._pending_orders.items() if value is order]

    def _claim_order_finalization(self, *_args): return True
    def _clear_restore_state(self, _order): pass
    ...
    def _clear_pending_order_refs(self, *ids):
        ...
        if order:
            for key in self._pending_keys_for_order(order): self._pending_orders.pop(key)
    def _consume_expected_cancellation(self, *ids):
        for order_id in ids: self._expected_cancellations.discard(str(order_id))
    async def _handle_exchange_order_object(self, _order): pass
    def get_pending_orders(self): return list(self._pending_orders.values())
...
class GridOwnershipTests(unittest.IsolatedAsyncioTestCase):
    async def test_only_owned_ids_reach_exchange(self):
        ...
        report = SimpleNamespace(
            requested={"301", "302"}, cancelled={"301", "302"}, filled=set(),
            still_open=set(), uncertain=set(), rejected={}, terminal_orders={},
        )
        engine.exchange.cancel_orders = AsyncMock(return_value=report)
        await grid.engine_cancel_orders(engine, [301, 302])
        engine.exchange.cancel_orders.assert_awaited_once_with(["301", "302"], "BTC")
...
class CoordinatorTests(unittest.IsolatedAsyncioTestCase):
    async def test_filter_uses_local_ids_not_exchange_side_scan(self):
        ...
        report = SimpleNamespace(
            requested={"501"}, cancelled={"501"}, filled=set(), still_open=set(),
            uncertain=set(), rejected={}, terminal_orders={},
        )
        engine.cancel_orders = AsyncMock(return_value=report)
        ops = SimpleNamespace(engine=engine, state=engine.coordinator.state, logger=MagicMock())
        ok = await grid.ops_cancel_filtered(
            ops, lambda order: order.side == "sell", "sell orders", 3
        )
        self.assertTrue(ok)
